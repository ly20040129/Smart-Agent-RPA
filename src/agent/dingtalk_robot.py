...
class DingtalkRobot:
    ...
    def send_file(self, file_path: str, userid: str) -> Dict:
        ...
        # 上传文件到钉钉，拿到 media_id（内部走 oapi 通道，用户无需理解）
        v1 = self._v1_token()
        if not v1: return {"success": False, "error": "上传文件失败（获取媒体token失败）"}
        media_id = self._upload(v1, file_path)
    ...
    # --- token（双轨：媒体上传用旧通道，发消息用新通道，钉钉本身限制，不用深究）---
    def _v1_token(self) -> Optional[str]:
        try:
            r = requests.get(
            ...
            logger.error(f"v1 token 网络错误: {e}"); return None
        if r.status_code != 200:
            logger.error(f"v1 token HTTP {r.status_code}: {r.text}"); return None
        d = r.json()
        if d.get("errcode") != 0:
            logger.error(f"v1 token 失败: {d}"); return None
    ...
    # --- 上传 / 发送 ---
    def _upload(self, v1_tok: str, file_path: str) -> Optional[str]:
        fname = os.path.basename(file_path)
        ...
        try:
            with open(file_path, 'rb') as f:
                r = requests.post(
                    ...
                    params={"access_token": v1_tok, "type": "file"},
