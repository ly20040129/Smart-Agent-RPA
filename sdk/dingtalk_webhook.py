# -*- coding: utf-8 -*-
"""
钉钉【群聊 Webhook 机器人】—— 发通知用的

用途：往群里发 文本/Markdown 通知（任务开始/成功/失败、告警等）
...
要给具体人发文件/文字，用 sdk/dingtalk_robot.py（单聊自建应用），两者凭证和用途完全不同。

用法：
    from sdk.dingtalk_webhook import build_webhook

    build_webhook().send_text("任务完成")
    build_webhook().notify_task_success("天猫上传", elapsed_sec=120)
"""
import json
import time
import hmac
...
class DingtalkWebhook:
    """群聊Webhook机器人：往群里发 文本/Markdown 通知"""
