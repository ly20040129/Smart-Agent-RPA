# -*- coding: utf-8 -*-
"""
钉钉【自建应用机器人】—— 给具体人发消息用的

用途：给具体人发 1对1 消息（文本/Markdown/文件），或向指定群发文件/文字
特点：单聊消息出现在"人与机器人"的会话窗口里（不是"工作通知"！）
凭证：app_key + app_secret（钉钉开发者后台 → 你的自建应用 → 凭证与基础信息里拿）
接收人：单聊用 dingtalk_userid（一串数字，如 "1783298686946923"，写在任务 yaml 的 params 里）；
        群发用 open_conversation_id + robot_code
配置：config.yaml → delivery.dingtalk 节点

只往群里发通知的话，用 sdk/dingtalk_webhook.py（群聊Webhook），两者凭证和用途完全不同。

用法：
    from sdk.dingtalk_robot import build_robot

    build_robot().send_file("D:/报表.xlsx", "178329xxx")       # 单聊发文件
    build_robot().send_text("完成", "178329xxx")               # 单聊发文字
    build_robot().send_group_file("D:/报表.xlsx", open_conversation_id, robot_code)
"""
import json
import os
import threading
...
class DingtalkRobot:
    """自建应用机器人：给具体钉钉 userid 发 1对1 消息（文本/Markdown/文件），或向指定群发送"""
    ...
    # ------------------------------------------------------------------
    # 群发（要 open_conversation_id + robot_code）
    # ------------------------------------------------------------------
